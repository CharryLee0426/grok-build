...
struct ChangesView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            }.font(.system(size: 11, design: .monospaced)).padding(.horizontal, 17).padding(.vertical, 15)
            if let error = store.workspace.error {
                ...
                HStack { Text("WORKING TREE").font(.system(size: 9, weight: .medium)).tracking(1); Spacer(); Text("\(store.workspace.changes.count) files").font(.system(size: 10)) }
                            ...
                            Button { Task { await store.selectFile(file.path) } } label: {
                                HStack(spacing: 9) {
                                    ...
                                    VStack(alignment: .leading, spacing: 4) {
                                        Text(URL(fileURLWithPath: file.path).lastPathComponent).font(.system(size: 11)).lineLimit(1)
                                        if file.path.contains("/") { Text((file.path as NSString).deletingLastPathComponent).font(.system(size: 9)).foregroundStyle(Theme.muted).lineLimit(1).truncationMode(.middle) }
                                    }
                                    Spacer(minLength: 2)
                                    if file.isBinary { Text("binary").font(.system(size: 9)).foregroundStyle(Theme.muted) }
                                ...
                                }.font(.system(size: 10, design: .monospaced)).padding(.horizontal, 10).padding(.vertical, 10).contentShape(Rectangle())
                ...
                if let selected = store.selectedFile {
                    ...
                    HStack { Text(URL(fileURLWithPath: selected).lastPathComponent).font(.system(size: 11, weight: .medium)).lineLimit(1); Spacer(); IconButton(icon: "xmark", help: "Close diff") { store.selectedFile = nil } }.padding(.horizontal, 18).padding(.vertical, 8)
                    ScrollView([.vertical, .horizontal]) {
                        VStack(alignment: .leading, spacing: 0) {
                            ForEach(Array(store.diffText.components(separatedBy: "\n").enumerated()), id: \.offset) { _, line in
                                Text(line.isEmpty ? " " : line).font(.system(size: 10, design: .monospaced)).textSelection(.enabled)
            ...
            Spacer(minLength: 0)
            HStack(spacing: 6) { Image(systemName: "arrow.triangle.branch"); Text(store.workspace.branch.isEmpty ? "Workspace" : store.workspace.branch).lineLimit(1); Spacer(); Text("Read-only review") }.font(.system(size: 10)).foregroundStyle(Theme.muted).padding(18).overlay(alignment: .top) { Theme.line.frame(height: 1) }
    ...
    private func empty(icon: String, title: String, subtitle: String) -> some View {
        VStack(spacing: 12) {
            ...
            Text(subtitle).font(.system(size: 11)).foregroundStyle(Theme.muted).multilineTextAlignment(.center).lineSpacing(3)
