...
final class Blake3Tests: XCTestCase {
    /// Official BLAKE3 test vectors: input byte `i` is `i % 251`; the hash is the default 32-byte output.
    ...
    func testOfficialVectors() {
        for (length, expected) in vectors {
            ...
            XCTAssertEqual(GrokBlake3.hex(input), expected, "input length \(length)")
        }
    }

    func testTextHashesItsUTF8Bytes() {
        XCTAssertEqual(GrokBlake3.hex(""), "af1349b9f5f9a1a6a0404dea36dcc9499bcb25c9adc112b7cc9a93cae41f3262")
        XCTAssertEqual(GrokBlake3.hex("## Käse\n"), GrokBlake3.hex(Data("## Käse\n".utf8)))
    }
}
